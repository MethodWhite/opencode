  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Task.create")(function* (input: CreateTaskInput) {
      ...
      const existing = yield* (db as any)
        ...
        .where(eq(TaskTable.session_id, input.session_id as any))
        .all()
        .pipe(Effect.orDie)
    ...
    const update = Effect.fn("Task.update")(function* (input: UpdateTaskInput) {
      ...
      yield* (db as any).update(TaskTable).set(updated).where(eq(TaskTable.id, input.id as any)).run().pipe(Effect.orDie)
    ...
    const del = Effect.fn("Task.delete")(function* (taskID: string) {
      ...
      if (!task) return
      yield* (db as any).delete(TaskTable).where(eq(TaskTable.id, taskID as any)).run().pipe(Effect.orDie)
      yield* events.publish(Event.Deleted, { sessionID: task.session_id, taskID })
    })

    const get = Effect.fn("Task.get")(function* (taskID: string) {
      const rows = yield* (db as any).select().from(TaskTable as any).where(eq(TaskTable.id, taskID as any)).all().pipe(Effect.orDie)
    ...
    const list = Effect.fn("Task.list")(function* (sessionID: SessionID, status?: TaskStatus, category?: TaskCategory) {
      const conditions = [eq(TaskTable.session_id, sessionID as any)]
    ...
    const listByParent = Effect.fn("Task.listByParent")(function* (parentID: string) {
      const rows = yield* (db as any).select().from(TaskTable as any).where(eq(TaskTable.parent_id, parentID as any)).orderBy(asc(TaskTable.position)).all().pipe(Effect.orDie)
      return rows.map(fromRow)
    })

    const reorder = Effect.fn("Task.reorder")(function* (sessionID: SessionID, taskIDs: string[]) {
      yield* (db as any).transaction((tx: any) =>
        Effect.gen(function* () {
          for (const [idx, id] of taskIDs.entries()) {
            yield* tx.update(TaskTable).set({ position: idx }).where(and(eq(TaskTable.id, id as any), eq(TaskTable.session_id, sessionID as any))).run()
          }
        }),
      ).pipe(Effect.orDie)
    ...
    const getBlocked = Effect.fn("Task.getBlocked")(function* (taskID: string) {
      const all: TaskInfo[] = yield* list("" as SessionID)
