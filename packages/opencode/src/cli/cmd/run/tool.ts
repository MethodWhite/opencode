...
import type { SwitchModeTool } from "@/tool/switch-mode"
...
type ToolDefs = {
  ...
  switch_mode: typeof SwitchModeTool
}
...
function runSwitchMode(p: ToolProps<typeof SwitchModeTool>): ToolInline {
  return {
    icon: "→",
    title: text(p.frame.state.title) || `Switching to ${p.input.mode ?? "agent"} mode`,
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  }
}
