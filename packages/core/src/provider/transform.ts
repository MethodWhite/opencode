...
import type * as ModelsDev from "../models-dev"
...
function googleThinkingVariants(model: Model): Record<string, Record<string, any>> {
...
export function variants(model: Model): Record<string, Record<string, any>> {
  ...
  switch (model.api.npm) {
    ...
    case "@ai-sdk/amazon-bedrock/mantle":
    case "@ai-sdk/openai":
      const efforts2 = openaiReasoningEfforts(model.api.id, model.release_date)
      return Object.fromEntries(
        efforts2.map((effort) => [
          effort,
          {
